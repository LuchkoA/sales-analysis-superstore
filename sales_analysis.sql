...
-- 4. Середній чек на замовлення - 457.44 у.од.
SELECT
    ROUND(SUM(sales) / COUNT(DISTINCT order_id), 2) AS avg_order_value
FROM orders;

-- 5. Продажі за регіонами: Найбільше продажів приносить регіон West з загальніми продажами 723 577 у.од.
SELECT
	region
    ,ROUND(SUM(sales), 2) AS total_sales
    ,ROUND(SUM(profit), 2) AS total_profit
...
-- 6. Категорії товарів з найвищим прибутком: найвищий прибуток дають товари категорії Technology
SELECT
	category
    ,ROUND(SUM(sales), 2) AS total_sales
    ,ROUND(SUM(profit), 2) AS total_profit
...
-- Частина 1 – Фінансова ефективність
-- 1. Маржа прибутку
